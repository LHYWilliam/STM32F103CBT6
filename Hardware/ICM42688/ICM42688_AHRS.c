...
void calGyroVariance(float data[], int length, float sqrResult[],
    ...
    if (GyroinitFlag == 0) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            sqrGyro_total[i] -=
                Gyro_fill[i][GyroCount] * Gyro_fill[i][GyroCount];
            Gyro_fill[i][GyroCount] = data[i];
            Gyro_total[i] += Gyro_fill[i][GyroCount];
            sqrGyro_total[i] +=
                Gyro_fill[i][GyroCount] * Gyro_fill[i][GyroCount];
    ...
    for (i = 0; i < 3; i++) {
        ...
        sqrResult[i] = (float)((sqrGyro_total[i] -
                                Gyro_total[i] * Gyro_total[i] / tmplen) /
                               tmplen);
    }
}
...
*******************************************************************************/
#define Kp \
    0.5f // proportional gain governs rate of convergence to
         // accelerometer/magnetometer
#define Ki \
    0.001f // integral gain governs rate of convergence of gyroscope biases
...
void IMU_AHRSupdate(float gx, float gy, float gz, float ax, float ay, float az,
  ...
  所以这里的vx\y\z，其实就是当前的欧拉角（即四元数）的机体坐标参照系上，换算出来的重力单位向量。
    */
    ...
    //  hx = 2*mx*(0.5f - q2q2 - q3q3) + 2*my*(q1q2 - q0q3) + 2*mz*(q1q3 +
    //  q0q2); hy = 2*mx*(q1q2 + q0q3) + 2*my*(0.5f - q1q1 - q3q3) + 2*mz*(q2q3
    //  - q0q1); hz = 2*mx*(q1q3 - q0q2) + 2*my*(q2q3 + q0q1) + 2*mz*(0.5f -
    //  q1q1 - q2q2); bx = sqrt((hx*hx) + (hy*hy)); bz = hz;
