...
// #define SPI_IMU_CS=0     HAL_GPIO_WritePin(ICM_PORT_SPIX_CS, ICM_PIN_SPIX_CS,
// GPIO_PIN_RESET) #define SPI_IMU_CS=1    HAL_GPIO_WritePin(ICM_PORT_SPIX_CS,
// ICM_PIN_SPIX_CS, GPIO_PIN_SET)
...
int8_t bsp_Icm42688RegCfg(void) {
    ...
    if (reg_val == ICM42688_ID) {
        //        icm42688_write_reg(ICM42688_REG_BANK_SEL, 1); //设置bank
        //        1区域寄存器 icm42688_write_reg(ICM42688_INTF_CONFIG4, 0x02);
        //        //设置为4线SPI通信

        //        icm42688_write_reg(ICM42688_REG_BANK_SEL, 0); //设置bank
        //        0区域寄存器 icm42688_write_reg(ICM42688_FIFO_CONFIG, 0x00);
        //        //Stream-to-FIFO Mode(page63)
        ...
        ////        icm42688_write_reg(ICM42688_FIFO_CONFIG1, 0x63); // Enable
        /// the accel and gyro to the FIFO
        ...
        icm42688_write_reg(ICM42688_REG_BANK_SEL, 0x00);
        reg_val = icm42688_read_reg(
            ICM42688_PWR_MGMT0); // 读取PWR—MGMT0当前寄存器的值(page72)
        reg_val &= ~(1 << 5);    // 使能温度测量
        ...
        ICM42688DelayMs(
            1); // 操作完PWR—MGMT0寄存器后 200us内不能有任何读写寄存器的操作
 ...
 * 备    注：
 *******************************************************************************/
int8_t bsp_Icm42688Init(void) { return (bsp_Icm42688RegCfg()); }
